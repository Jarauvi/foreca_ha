...
class ForecaDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass: HomeAssistant, url: str, snow_station: str, lake_station: str, ice_station: str):
    ...
    async def _async_update_data(self):
        ...
        try:
            async with session.get(self.url, headers=HEADERS, timeout=10) as response:
                if response.status != 200:
                    raise UpdateFailed(f"Error fetching Foreca page: {response.status}")
        ...
        try:
            async with session.get(LAKE_TEMP_URL, headers=HEADERS, timeout=10) as response:
                if response.status == 200:
        ...
        try:
            async with session.get(ICE_URL, headers=HEADERS, timeout=10) as response:
                if response.status == 200:
                    json_data = await response.json()
                    stations = json_data.get("stations", [])
                    ...
                    if s_id:
                        obs_list = json_data.get("observations", {}).get(s_id, [])
                        # Ice data is a list of observations. We find the latest non-null value.
                        latest_obs = None
        ...
        try:
            for title in soup.find_all("li", class_="title"):
                ...
                key_prefix = next((v for k, v in LABEL_MAP.items() if k in title_text), None)
                if not key_prefix:
                    continue
                ...
                if key_prefix == "warnings":
                    ...
                    # Try to convert to float for measurements by stripping non-numeric junk
